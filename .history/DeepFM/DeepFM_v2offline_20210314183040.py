...
def get_similar_items(item2vec,item_features,item_id,cold_start=None):
    ...
    return sim_dict
def Retrieve(retrieve_model,vectorizer,user_item,topK=36):
    encode_dict=dict()
    item_vectors=dict()
    user_pooling=[]
...
def retrain(model,data,epochs=3,learning_rate=0.01):
    ...
    return model
def guess_you_like(model,vectorizer,X,topK=36,json_like=True,predict_type='single'):
        for key in user_fields:
            if len(X[key])!=len(X['item_id']):
                X[key]=np.repeat(X[key],len(X['item_id']))
        X_vectorize=vectorize(vectorizer,X)
        pred=model.predict(X_vectorize)
        df=pd.DataFrame(X)
        df['score']=pred
