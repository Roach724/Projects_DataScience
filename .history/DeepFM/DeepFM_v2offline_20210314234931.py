...
def DeepFM(input_dim=19,hidden_units=[512,256,128],activation='relu',dropouts=[0.2,0.3,0.4],l2_regularization=0.1):
    if len(hidden_units)!=len(dropouts):
        ...
        return 0
    inputs=tf.keras.layers.Input(shape=(input_dim,),dtype=tf.float32,name='input_feature')
    #DNN
    DNN=tf.keras.Sequential(name='DNN')
...
def create_feature(vectorizer,retriever,data,user_log,rank_data,N=10):
    item_embedding=get_item_embedding(vectorizer,retriever,rank_data)
