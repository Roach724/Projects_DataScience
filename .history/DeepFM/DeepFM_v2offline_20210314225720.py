...
class Crosslayer(tf.keras.layers.Layer):
    ...
    def call(self,inputs):
        ...
        diff=0.5*(square_of_sum-sum_of_square)
        return diff
    def get_config(self):
        config=super(Crosslayer,self).get_config()
        return config
def DeepFM(hidden_units=[512,256,128],dropouts=[0.4,0.5,0.6]):
    ...
    for i,layer_size in enumerate(hidden_units):
        DNN.add(tf.keras.layers.Dense(layer_size,kernel_regularizer=tf.keras.regularizers.l2(l2_regularization),
                        ...
                        kernel_initializer=tf.keras.initializers.glorot_normal(),activation='tanh',name='dnn_layer'+str(i+1)))
