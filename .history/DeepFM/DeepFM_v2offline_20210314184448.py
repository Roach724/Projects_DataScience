...
def Retrieval():
    ...
    for layer_size in [16,16,16,embedding_dim]:
...
def DeepFM():
    ...
    dnn1=dnn_l1(dense_features_flatten_context)
...
def retrain(ranker,data,epochs=3,learning_rate=0.01):
    ...
    return ranker
def guess_you_like(ranker,vectorizer,X,topK=36,json_like=True,predict_type='single'):
        for key in user_fields:
            if len(X[key])!=len(X['item_id']):
                X[key]=np.repeat(X[key],len(X['item_id']))
        X_vectorize=vectorize(vectorizer,X)
        pred=ranker.predict(X_vectorize)
        df=pd.DataFrame(X)
        df['score']=pred
