...
#model parameters
embedding_dim=8
retriever_hidden_units=[16,16,16,embedding_dim]
ranker_hidden_units=[256,256,256,256]
ranker_dropout=[0.4,0.5,0.6,0.7]
...
def cross_validation(X,y,train_batch_size=16932,test_batch_size=4096,epochs=5,cv=2,n_splits=5,shuffle=True,random_state=None):
    ...
    for i in range(cv):
        for train_idx,test_idx in kf.split(X_df):
            ...
            vectorizer.get_layer(name='item_tag_vectorize').adapt(np.unique(X['item_tag']))
            vectorizer.get_layer(name='user_id_vectorize').adapt(np.unique(X['user_id']))
            vectorizer.get_layer(name='item_id_vectorize').adapt(np.unique(X['item_id']))
            vectorizer.get_layer(name='member_type_vectorize').adapt(np.unique(X['member_type']))
            vectorizer.get_layer(name='user_type_vectorize').adapt(np.unique(X['user_type']))
            vectorizer.get_layer(name='item_catalog_intlookup').adapt(np.unique(X['item_catalog']))
...
def get_similar_items(item2vec,item_features,item_id,cold_start=None):
    ...
    return sim_dict
def Retrieve(retriever,vectorizer,user_item,topK=36):
    encode_dict=dict()
    item_vectors=dict()
    user_pooling=[]
    user_item_encode=vectorizer(user_item)
    ...
    for key in encode_dict.keys():
        if key in user_fields:
            user_pooling.append(retriever.get_layer(key+'_embedding')(encode_dict[key]).numpy()[0,:,:])
        if key in item_fields:
            item_vectors.setdefault(key,0)
            item_vectors[key]=retriever.get_layer(key+'_embedding')(encode_dict[key]).numpy()
    retrieval_dict=dict()
...
def fit_vectorizer(vectorizer,X):
    ...
    vectorizer.get_layer(name='member_type_vectorize').adapt(np.unique(X['member_type']))
    vectorizer.get_layer(name='user_type_vectorize').adapt(np.unique(X['user_type']))
    vectorizer.get_layer(name='item_catalog_vectorize').adapt(np.unique(X['item_catalog']))
    vectorizer.compile()
    return vectorizer
...
def Retrieval():
    ...
    for layer_size in retriever_hidden_units:
...
def DeepFM():
    ...
    for i,layer_size in enumerate(ranker_hidden_units):
        ...
        DNN.add(tf.keras.layers.Dropout(ranker_dropout[i]))
