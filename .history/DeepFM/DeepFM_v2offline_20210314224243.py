...
def get_item_vector(item2vec,item_features,item_id,weights={'age_lower':2,'age_upper':2,'tax_rating':3,
    ...
    for col in cat_col+num_col:
        ...
        if col in str_col:
            vector_dict.setdefault(col+'_embedding',0)
            vector_dict[col+'_embedding']=item2vec.get_layer(col+'_embedding')(item2vec.get_layer(col+'_vectorize')(item_feature)).numpy()[0,:,:]
        elif col in int_col:
            vector_dict.setdefault(col+'_embedding',0)
            vector_dict[col+'_embedding']=item2vec.get_layer(col+'_embedding')(item2vec.get_layer(col+'_vectorize')(item_feature)).numpy()
        else:
            vector_dict.setdefault(col,0)
            vector_dict[col]=item_feature.reshape(1,-1)
    for key in weights.keys():
        if key in cat_col:
            vector_dict[key+'_embedding']=weights[key]*vector_dict[key+'_embedding']
        else:
            vector_dict[key]=weights[key]*vector_dict[key]
    vector_array=[]
    for col in cat_col:
        vector_array.append(vector_dict[col+'_embedding'])
...
class Crosslayer(tf.keras.layers.Layer):
    ...
    def call(self,inputs):
        square_of_sum=tf.keras.backend.square(tf.keras.backend.sum(inputs,axis=0,keepdims=True))
        sum_of_square=tf.keras.backend.sum(tf.keras.backend.square(inputs),axis=0,keepdims=True)
...
def DeepFM(hidden_units=[256,256,256,256],dropouts=[0.4,0.5,0.6,0.7]):
    if len(hidden_units)!=len(dropouts):
        ...
        return 0
    inputs=tf.keras.layers.Input(shape=(27,),dtype=tf.float32,name='input_feature')
    #DNN
    DNN=tf.keras.Sequential(name='DNN')
    ...
    DNN.add(tf.keras.layers.Dense(1,kernel_regularizer=tf.keras.regularizers.l2(l2_regularization),use_bias=False,
                        bias_regularizer=tf.keras.regularizers.l2(l2_regularization),
                        kernel_initializer=tf.keras.initializers.glorot_normal(),activation=None,name='dnn_layer'+str(i+2)))
    #FM
    fm_linear=tf.keras.backend.sum(inputs,axis=1)
