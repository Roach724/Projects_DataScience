...
class LatentFactors:
    ...
    def predict(self,data):
        ...
        for j in range(n):
            ...
            V[:,j,:]=v
            W[:,j]=w
        logit=np.sum(W,axis=1)+0.5*(np.linalg.norm(np.sum(V,axis=1),axis=1)-np.sum(np.linalg.norm(V,axis=2),axis=1))
    ...
    def fit(self,data,item_pool,ratio=5,num_iterations=5,learning_rate=0.01,verbose=True,init_model=None,debug=False):
        ...
        for t in range(num_iterations):
            ...
            for i in range(m):
                ...
                g0=y_hat-y[i]
                values=data[i]
                features_idx=np.isin(self.feature_list[:,0],self.feature_cols)
                values_idx=[np.argmax(self.feature_list[features_idx,1]==v) for v in values]
                np.isin()
                for j in range(n):
                    feature=self.feature_cols[j]
                    value=[data[i,j]]
                    feature_idx=np.isin(self.feature_list[:,0],feature)
                    value_idx=[np.argmax(self.feature_list[feature_idx,1]==v) for v in value]

                    for k in range(self.K):
                        inter1=np.sum(self.embedding_matrix[features_idx][values_idx,k])
                        g1=inter1-self.embedding_matrix[feature_idx][value_idx,k]
                        self.embedding_matrix[feature_idx][value_idx,k]-=learning_rate*g0*g1
                    self.W[feature_idx][value_idx]-=learning_rate*g0
                y_hat=self.predict(data[i])
